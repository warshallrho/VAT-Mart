...
def run_an_RL(idx_process, args, transition_Q, epoch_Q, decay_Q):
    ...
    valid_list = [2 for idx in range(len_all_shape_list + 10)]
    ...
    for epoch in range(EP_MAX):
        ...
        if args.all_shape:
            idx_shape = random.randint(0, len_all_shape_list - 1)
            while valid_list[idx_shape] == 0:
                idx_shape = random.randint(0, len_all_shape_list - 1)
            shape_id = all_shape_list[idx_shape]
        else:
        ...
        joint_angles = env.load_object(object_urdf_fn, object_material, state=target_part_state,
                                                              target_part_id=-1)
        env.render()
        if valid_list[idx_shape] == 2:
            if env.check_drawer():
                valid_list[idx_shape] = 1
            else:
                valid_list[idx_shape] = 0
        if valid_list[idx_shape] == 0:
        ...
        task_upper = min(joint_angle_upper, args.task_upper)
        ...
        try:
            ...
            for step_idx in range(num_steps):
                ...
                if distance < 0.1 * abs(task_radian):  # bonus
                    if args.early_stop or args.every_bonus:
                    ...
                    try:
                        imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%d.gif' % (tot_done_epoch, idx_process, task_radian, init_target_part_qpos, joint_angle_upper, step_idx+1)), succ_images)
                    except:
                        pass
                    ...
                    if tot_fail_epoch < tot_done_epoch:
                        tot_fail_epoch += 1
                        try:
                            imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%.3f_%d.gif' % (tot_fail_epoch, idx_process, task_radian, init_target_part_qpos, joint_angle_upper, distance, step_idx+1)), succ_images)
                        except:
                            pass

                # store (s,a,r)
                if args.early_stop:
                    if stop and distance < 0.1 * abs(task_radian):
                        success_traj = True
                    ...
                    finish = (step_idx == num_steps - 1)
                    if finish and (distance < 0.1 * abs(task_radian)):
                        success_traj = True
                    replay_buffer.push(cur_state.view(-1).cpu().detach().numpy(), next_qpos.view(-1).cpu().detach().numpy(), reward,
                                       next_state.view(-1).cpu().detach().numpy(), done=finish)
    ...
    if not trans_q.empty():
        ...
        if args.use_HER:
            ...
            for i, transition in enumerate(transition_list):
                ...
                if abs(cur_position - next_position) < args.HER_move_margin:
                    continue
                if args.state_initial_position:
                ...
                for new_goal in new_goals:
                    ...
                    if abs(next_position - new_goal) < args.target_margin:
                        # bonus
                        if args.early_stop or args.every_bonus:
                            reward += args.success_reward
                        else:
                    ...
                    if args.HER_only_success:
                        if done:
                            if args.early_stop:
                                ...
                                replay_buffer.push(new_cur_state, action, reward, new_next_state, finish)
                    else:
                        if args.early_stop:
                            replay_buffer.push(new_cur_state, action, reward, new_next_state, done)
                        else:
        ...
        print(
            'Episode: {}/{}  | Task: {:.4f}  | Final Distance: {:.4f}  | Episode Reward: {:.4f}  | Episode Final Reward: {:.4f}  | Bonus: {:.4f}  | Penalty: {:.4f}  | Guidance: {:.4f}  | Accuracy: {:.4f} | Running Time: {:.4f}'.format(
                epoch, EP_MAX, task_degree, final_distance, ep_r, final_reward, bonus, tot_penalty, tot_guidance, accuracy,
                time.time() - t0
            )
        )
