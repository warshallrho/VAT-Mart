...
def run_an_RL(idx_process, args, transition_Q, epoch_Q, decay_Q):
    ...
    for epoch in range(EP_MAX):
        ...
        env.flog = flog
        selected_cat = all_cat_list[random.randint(0, tot_cat - 1)]
        ...
        try:
            ...
            for step_idx in range(num_steps):
                ...
                if distance < degree2radian(abs(task_degree) * 0.15):  # bonus
                    ...
                    try:
                        imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%d_%s_%s.gif' % (tot_done_epoch, idx_process, task_degree, task_upper, radian2degree(distance), step_idx+1, shape_cat_dict[shape_id], shape_id)), succ_images)
                    except:
                        pass
                    ...
                    if tot_fail_epoch < tot_done_epoch * 100:
                        tot_fail_epoch += 1
                        try:
                            imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%d_%s_%s.gif' % (tot_fail_epoch, idx_process, task_degree, task_upper, radian2degree(distance), step_idx+1, shape_cat_dict[shape_id], shape_id)), succ_images)
                        except:
                            pass
        ...
        if args.use_HER:
            ...
            for i, transition in enumerate(transition_list):
                ...
                for new_goal in new_goals:
                    ...
                    if abs(next_position - new_goal) < abs(new_task) * 0.15:
                        if args.early_stop:
                            reward += args.success_reward
                        else:
                            if cur_state[-2] == 1:
