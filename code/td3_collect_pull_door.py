...
parser.add_argument('--with_step', type=int, default=0)
...
parser.add_argument('--RL_pretrained_id', type=int, default=0, help='xxx')
...
def run_an_RL(idx_process, args, transition_Q, epoch_Q, decay_Q):
    ...
    for epoch in range(EP_MAX):
        ...
        tot_trial = 0
        while tot_trial < 50 and ((env.target_object_part_joint_type != ArticulationJointType.REVOLUTE) or (env.get_target_part_axes_dir_new(target_part_id) == 1)):
            idx = np.random.randint(len(xs))
            x, y = xs[idx], ys[idx]
            target_part_id = object_link_ids[gt_movable_link_mask[x, y] - 1]
            env.set_target_object_part_actor_id(target_part_id)
            tot_trial += 1
        if (env.target_object_part_joint_type != ArticulationJointType.REVOLUTE) or (env.get_target_part_axes_dir_new(target_part_id) == 1):
            env.scene.remove_articulation(env.object)
            flog.close()
            continue
            ...
            for step_idx in range(num_steps):
                ...
                try:
                    if args.vis_gif:
                        ...
                        cur_waypoints = robot.move_to_target_pose(final_rotmat, 3000, cam=cam, vis_gif=False, vis_gif_interval=500, visu=True)
                ...
                if distance < degree2radian(abs(task_degree) * 0.1):  # bonus
                    ...
                    if args.vis_gif:
                        try:
                            imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%d.gif' % (tot_done_epoch, idx_process, task_degree, task_upper, radian2degree(distance), step_idx+1)), succ_images)
                        except:
                            pass
                    ...
                    if args.vis_gif:
                        if tot_fail_epoch < tot_done_epoch * 100:
                            tot_fail_epoch += 1
                            try:
                                imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%d.gif' % (tot_fail_epoch, idx_process, task_degree, task_upper, radian2degree(distance), step_idx+1)), succ_images)
                            except:
                                pass
