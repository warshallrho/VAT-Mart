...
parser.add_argument('--pn_feat', action='store_true', default=False, help='xxx')
...
parser.add_argument('--closeDoor_uplimit', type=int, default=200)
...
def run_an_RL(idx_process, args, transition_Q, epoch_Q, decay_Q):
    ...
    print("args.guidance_reward:", args.guidance_reward)
    ...
    for epoch in range(EP_MAX):
        ...
        while tot_trial < 50 and not found_task:
            ...
            tot_trial += 1
            if degree_record[int(task_radian * 10) - 1] < args.degree_uplimit:
                found_task = True
                break
        if not found_task:
            continue
        ...
        try:
            ...
            if args.with_step:
                state_RL.extend(step_one_hot[0])
            ...
            for step_idx in range(num_steps):
                ...
                try:
                    if args.vis_gif:
                        ...
                        cur_waypoints = robot.move_to_target_pose(final_rotmat, 3000, cam=cam, vis_gif=False, visu=True)
                ...
                if args.with_step:
                    state_.extend(step_one_hot[step_idx + 1])
                ...
                if distance < abs(task_radian) * 0.10:  # bonus
                    ...
                    if args.vis_gif:
                        try:
                            imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%d.gif' % (tot_done_epoch, idx_process, radian2degree(task_radian), radian2degree(init_target_part_qpos), radian2degree(joint_angle_upper), step_idx+1)), succ_images)
                        except:
                            pass
                    ...
                    if tot_fail_epoch < tot_done_epoch:
                        ...
                        if args.vis_gif:
                            try:
                                imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%.3f_%d.gif' % (tot_fail_epoch, idx_process, radian2degree(task_radian), radian2degree(init_target_part_qpos), radian2degree(joint_angle_upper), radian2degree(distance), step_idx+1)), succ_images)
                            except:
                                pass

                # store (s,a,r)
                if stop and distance < abs(task_radian) * 0.10:
                    success_traj = True
...
while True:
    if not trans_q.empty():
        ...
        print(
            'Episode: {}/{}  | Final Distance: {:.4f}  | Episode Reward: {:.4f}  | Episode Final Reward: {:.4f}  | Penalty: {:.4f}  | Accuracy: {:.4f} | Running Time: {:.4f}'.format(
                epoch, EP_MAX, final_distance, ep_r, final_reward, tot_penalty, accuracy,
                time.time() - t0
            )
        )
