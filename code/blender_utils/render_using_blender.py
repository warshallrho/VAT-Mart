...
def render_pts(out_fn, pts, blender_fn='camera.blend', highlight_id=None):
    ...
    cmd = 'cd %s && /home/user/blender-2.79-linux-glibc219-x86_64/blender -noaudio --background %s --python render_blender.py %s %s > /dev/null' \
            % (os.path.join(os.path.dirname(os.path.abspath(__file__))), \
            blender_fn, out_fn+'.obj', out_fn)
    # print("cmd:", cmd)
    call(cmd, shell=True)

def render_pts_highlight_points(out_fn, pts, blender_fn='camera.blend', highlight_points=[]):
    all_v = [np.zeros((0, 3), dtype=np.float32)];
    all_f = [np.zeros((0, 3), dtype=np.int32)];
    t = 0
    for i in range(pts.shape[0]):
    ...
    with open(out_fn+'.obj', 'w') as fout:
        ...
        if len(highlight_points) != 0:
            for hightlight_point in highlight_points:
                vs = cube_v * 5 + hightlight_point
                fs = cube_f + all_v.shape[0]
                for i in range(vs.shape[0]):
                    fout.write('v %f %f %f\n' % (vs[i, 0], vs[i, 1], vs[i, 2]))
                fout.write('usemtl f1\n')
