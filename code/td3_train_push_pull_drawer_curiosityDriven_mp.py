...
parser.add_argument('--critic_log_dir', type=str, default='/home/user/VAT_Mart/VAT_Data', help='exp logs directory')
...
parser.add_argument('--push_drawer', type=int, default=0)
...
def get_critic_score(input_pcs, up, forward, contact_point, waypoints, task_degree):
    ...
    # input_pcid2 = furthest_point_sample(input_pcs.contiguous(), torch.tensor(args.num_point_per_shape).to(device)).long().reshape(-1)  # BN
...
def run_an_RL(idx_process, args, transition_Q, epoch_Q, decay_Q):
    ...
    for epoch in range(EP_MAX):
        ...
        joint_angle_upper_radian = joint_angle_upper - joint_angle_lower
        task_upper = min(joint_angle_upper_radian, args.task_upper)
        ...
        if args.primact_type == 'pulling':
            joint_angles = env.update_joint_angle(joint_angles, target_part_joint_idx, target_part_state,
                                                  task_radian, push=False, pull=True, drawer=True)
        else:
            joint_angles = env.update_joint_angle(joint_angles, target_part_joint_idx, target_part_state, task_radian, push=True, pull=False, drawer=True)
        ...
        try:
            ...
            for step_idx in range(num_steps):
                ...
                if not success:
                    break
                waypoints.append(robot.robot.get_qpos().tolist())
                target_part_trans = env.get_target_part_pose().to_transformation_matrix()   # 得到世界坐标系 -> target part坐标系的 transformation matrix
                now_contact_point_position = target_part_trans @ position_local_xyz1    # 得到contact point在世界坐标系下的位置
                ...
                if np.linalg.norm(now_contact_point_position - now_gripper_finger_position) > threshold_gripper_distance:
                    reward -= 300
                    tot_penalty -= 300
                if np.linalg.norm(now_contact_point_position - now_gripper_finger_position) > 0.2:
                ...
                if distance < abs(task_radian) * 0.1:  # bonus
                    ...
                    try:
                        imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%d_%s.gif' % (tot_done_epoch, idx_process, task_radian, radian2degree(init_target_part_qpos), radian2degree(joint_angle_upper), step_idx+1, selected_cat)), succ_images)
                    except:
                        pass
                    ...
                    if tot_fail_epoch < tot_done_epoch:
                        tot_fail_epoch += 1
                        try:
                            imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%.3f_%d_%s.gif' % (tot_fail_epoch, idx_process, task_radian, radian2degree(init_target_part_qpos), radian2degree(joint_angle_upper), radian2degree(distance), step_idx+1, selected_cat)), succ_images)
                        except:
                            pass
...
if args.continue_to_play:
    network.load_state_dict(torch.load(os.path.join(out_dir, 'critic_ckpts', '%s-network.pth' % args.critic_load_epoch)))
else:
...
epoch = -1
if args.continue_to_play:
    epoch = int(args.critic_load_epoch) - 1
...
while True:
    if not trans_q.empty():
        ...
        task_degree = degree2radian(task_radian)
        ...
        if args.use_HER:
            ...
            for i, transition in enumerate(transition_list):
                ...
                for new_goal in new_goals:
                    ...
                    if abs(next_position - new_goal) < 0.1 * abs(task_radian):
                        # bonus
                        reward += args.success_reward
                        done = True

                        new_task_radian = new_goal - init_target_part_qpos
                        new_task_degree = radian2degree(new_task_radian)
