...
parser.add_argument('--pred_world_xyz', type=int, default=1)
parser.add_argument('--pred_residual_world_xyz', type=int, default=1)
parser.add_argument('--pred_residual_root_qpos', type=int, default=0)
        ...
        try:
            ...
            for step_idx in range(num_steps):
                ...
                if distance < degree2radian(abs(task_degree) * task_succ_margin):  # bonus
                    ...
                    try:
                        imageio.mimsave(os.path.join(result_succ_dir, '%d_%d_%.3f_%.3f_%.3f_%.3f_%d_%s_%s_%.3f.gif' % (tot_done_epoch, idx_process, task_degree, radian2degree(init_target_part_qpos), radian2degree(joint_angle_lower), radian2degree(joint_angle_upper), step_idx+1, selected_cat, shape_id, gripper_ctpt_distance)), succ_images)
                    except:
                        pass
                    ...
                    if tot_fail_epoch < tot_done_epoch:
                        tot_fail_epoch += 1
                        try:
                            imageio.mimsave(os.path.join(result_fail_dir, '%d_%d_%.3f_%.3f_%.3f_%.3f_%.3f_%d_%s_%s_%.3f.gif' % (tot_fail_epoch, idx_process, task_degree, radian2degree(init_target_part_qpos), radian2degree(joint_angle_lower), radian2degree(joint_angle_upper), radian2degree(distance), step_idx+1, selected_cat, shape_id, gripper_ctpt_distance)), succ_images)
                        except:
                            pass

                # store (s,a,r)
                if args.early_stop:
                    if stop and distance < degree2radian(abs(task_degree) * 0.15):
                        success_traj = True
                    ...
                    if finish and distance < degree2radian(abs(task_degree) * 0.15):
                        success_traj = True
                    replay_buffer.push(cur_state.view(-1).cpu().detach().numpy(), next_qpos.view(-1).cpu().detach().numpy(), reward,
        ...
        if args.use_HER:
            ...
            for i, transition in enumerate(transition_list):
                ...
                if args.state_initial_position:
                    ...
                    now_gripper_finger_position = next_state[9: 12]
                else:
                    now_contact_point_position = next_state[3: 6]
                    now_gripper_finger_position = next_state[6: 9]
